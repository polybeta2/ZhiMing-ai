...
- **撰写模式**：续写支持 800/1500/2500 快捷档与自由档字长选择；自动撰写流水线一键生成整卷
- **文风蒸馏与风格卡**：导入小说样本 → 六层机制分析 → 风格卡（句法/语感/反 AI 规则）→ 写作/大纲/写作助手按预算装填注入；支持多档案按层融合、反 AI 味自动约束、风格体检、JSON 导入导出
- **原作档案库（同人地基）**：导入 txt 自动识别编码 → 快扫/精扫深度分析 → 人物卡 / 事件时间线 / 世界观档案；扫描流式可视化、断点续跑、失败自动重试 3 次、剩余时间估算；LiveContainer 可用 origins 文件夹导入
- **同人立项**：选择原作档案 → 澄清遵守度（严格原作线 / IF 线 / 仅借设定）→ 文风可选（继承原作文风 / 不启用 / 其他小说文风）→ 全书阶段时间窗注入，确保不 OOC
- **批量复制分析（省钱通道）**：把最烧 token 的章节分析外包给免费外部 AI（DeepSeek App / Gemini 等）——复制每批 N 章提示词 → 外部分析 → 粘贴回填 → 织命内归并建档；API 自动分析支持 1~10 章/请求批量
- **小说续写 / IF 线**：导入断更 / 烂尾 / 自己未写完的 txt，浏览全部章节选「从第 X 章续写」（默认最后一章；选中间章 = 写自己的 IF 线）；深度分析 1~X 章（人物现状快照 / 未回收伏笔 / 剧情弧）→ 自动建书进入续写蓝图 → 写作时滚动注入近期原文作为文风锚点
- **自适应多行输入**：聊天框 / 蓝图卡 / 大纲字段 / 风格规则全链路多行自适应（最多 5 行内滚），iOS 15 同样可用
...
- **特别感谢所有国产大模型的辛勤付出** —— 本项目从工程搭建、逐迭代编码到最终打包交付，全程由 DeepSeek / Qwen 等国产大模型协作完成。
